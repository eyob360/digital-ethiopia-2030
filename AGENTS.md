...
- ID reservation: sequential IDs are allocated by committing the new item's index line (status `draft`) plus its stub file directly to `main` and pushing immediately — the push is the lock; if rejected, pull and take the next number. This is the one allowed direct-to-`main` commit type: docs-only reservations, never code. No remote? Fetch-and-scan open branches before minting, and renumber the unmerged side on collision. Migration numbering: use the stack's merge tooling (e.g. `makemigrations --merge`), named in Commands & stack.
