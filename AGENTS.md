...
1. **Orient first.** Read `workflow/STATE.md` before starting any work. If the `Last drift audit` line is stale (~20+ commits behind, or none on a non-trivial codebase), mention it to the user at the next natural pause — don't wait to be asked "what's next".
2. **Ask, don't assume.** If requirements, context, or a decision are missing or ambiguous, ask the user. Record every answer in `workflow/decisions/` and index it in `workflow/DECISIONS.md`. Check existing decisions first — never re-ask, and check before proposing an approach (it may be a recorded dead end). Chat is not a record: any question for the user still open when your session ends must be written under **Blocked / Needs user input** in `workflow/STATE.md` before you finish.
...
- ID reservation: sequential IDs are reserved by pushing the item's index line + stub straight to `main` (docs-only — the one allowed direct-to-`main` commit; stub = the folder's template file with frontmatter filled, `status: draft`, body may be skeletal). If the push is rejected, pull/rebase, then **verify your ID is still unique in the index** — same-ID lines can auto-merge without a conflict — renumber if taken, push again. No remote: fetch-and-scan open branches, renumber on collision at merge. Migration numbering: stack merge tooling (e.g. `makemigrations --merge`), named in Commands & stack.
