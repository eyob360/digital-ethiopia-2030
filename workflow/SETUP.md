---
status: in-progress
type:            # web-app | cli | data-pipeline | mobile | library | ... (informational for now)
modules: []      # opted-in extras beyond the core pipeline — see modules/INSTRUCTIONS.md
...
- [ ] Ensure the project is a git repo (`git init` + first commit if not) — drift audits and evidence reconciliation depend on it
- [ ] Confirm each agent tool in use reads `AGENTS.md`; add symlinks for tools that expect their own filename (`CLAUDE.md` ships pre-linked)
- [ ] Fill the profile frontmatter above with the user: project `type`, and which optional `modules` this project opts into (present the options in `modules/INSTRUCTIONS.md`)

- [ ] Ask the user which repository layout they want, explaining the options:
...
- [ ] Review **Conventions** in `AGENTS.md` with the user; adjust defaults
- [ ] Write `brds/OVERVIEW.md` with the user (product-level context; see `brds/INSTRUCTIONS.md`)
- [ ] Obtain requirements: ask the user for SRS/requirement document(s) and get them into `srs/` (see "Receiving documents" in `srs/INSTRUCTIONS.md`). If none exists, gather requirements in conversation instead (see `brds/INSTRUCTIONS.md`).
