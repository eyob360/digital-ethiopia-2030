...
- WO-0006 — re-validation 2 failed (VAL-WO-0006): budget-exhausted terminal items still enter the fetch pipeline (clear `url` or route to Complete); no-priority-URL lineages get an independent budget (~20 docs/run possible) — fix app-side counter, or user decision to accept per-branch budget
